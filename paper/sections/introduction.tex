...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{Finding (3 seeds, 3 checkpoints).}
    We find that L1H6's prefix-matching score \emph{increases} under both
    fine-tuning conditions: $0.408 \to 0.591 \pm 0.005 \to 0.646 \pm 0.007$
    (code) and $0.408 \to 0.583 \pm 0.002 \to 0.616 \pm 0.001$ (prose)
    over steps $0/100/200$ (mean $\pm$ SD across seeds 42, 123, 7), with code
    producing a significantly larger increase by step 200
    ($+0.030$, $4.1\times$ pooled SD)---a result not reported in any
    prior work---implying that the induction circuit is not silently eroded
    by fine-tuning at this token budget, that the magnitude of reinforcement
    is domain-dependent, and that safety evaluations should include
    circuit-level diagnostics to detect both degradation \emph{and}
