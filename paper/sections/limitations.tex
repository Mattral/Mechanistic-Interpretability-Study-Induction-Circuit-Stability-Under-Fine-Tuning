...
\textbf{Single seed analysed (fine-tuning).}
All fine-tuning trajectories (Figures~5--7, Tables~1--3) are from seed 42
only. Seeds 123 and 7 were trained and checkpoints exist, but the checkpoint
sweep could not be completed before Colab session resets. No $\pm 1$~SD
confidence intervals are available for the IS trajectories; the $\Delta$ and
IS values in Tables~1--3 should be treated as single-seed observations
